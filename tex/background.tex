...
To capture the structure of a program, syntactic representations such as Abstract Syntax Trees (ASTs) are used. ASTs encode the hierarchical structure of programs by representing code as a tree, where each node corresponds to a language construct such as a method, loop, or variable declaration. This structure enables the extraction of rich syntactic features, which, when combined with hierarchical relationships, makes ASTs particularly effective for tree-based similarity measures such as tree kernels and subtree matching. Such representations can benefit tasks like clone detection~\cite{deckard2007}, where structural similarities indicate potential code duplication or reuse. However, ASTs do not explicitly encode program semantics, which limits their ability to identify semantically equivalent or similar code.

\subsection{Semantic Representations}

Unlike syntactic representations, semantic representations focus on modeling relationships that reflect program behavior rather than code structure alone. They encode how program elements interact or depend on one another, providing a higher-level view of program functionality. Earlier semantic representations included graph-based models such as program dependence graphs (PDGs~\cite{komondoor2001using}) and data-flow graphs (DFGs), which encode control and data dependencies explicitly. Modern graph-based embeddings can be seen as a learned extension of these approaches.

With the rise of deep learning, learned representations of code, commonly referred to as embeddings, have become pervasive. These representations emphasize behavioral similarity rather than syntactic form and are designed to capture relationships between code elements that are not apparent from syntax alone. Embeddings are typically derived from neural language models pre-trained on a large corpora of source code. These can be broadly categorized based on the underlying model architecture, including sequence-based models (e.g., RNNs, LSTMs), transformer-based models (e.g., CodeBERT~\cite{arshad2022codebert}, GraphCodeBERT~\cite{guo2020graphcodebert}), and graph-based models (e.g., those using Graph Neural Networks). Code embeddings help bridge vocabulary mismatches in code search by abstracting away from surface-level textual differences and focusing on deeper semantic intent. Moreover, they can be fine-tuned for specific downstream tasks and have shown superior performance in applications such as code summarization~\cite{allamanis2016convolutional}, and clone detection~\cite{komondoor2001using,white2016deep}.
...
\(k_1 ∈ [1.2,2.0]\) and \(b=0.75\). \({IDF}(q_i)\)
...
Structural similarity approaches represent source code as trees such as ASTs and enable capturing syntactic relationships based on tree comparison rather than token-based comparison.


Techniques like tree kernels and tree edit distance enable the comparison of code segments based on their hierarchical, syntactic relationships rather than their textual tokens. Tree kernels quantify similarity by capturing shared subtrees within ASTs, enabling the detection of functionally similar code. Tree edit distance, on the other hand, measures the minimum number of edit operations (insertions, deletions, or modifications) needed to transform one tree into another. This makes it particularly useful for fine-grained comparison of structural differences in code. Graph based techniques that represent program as PDGs~\cite{komondoor2001using} use graph isomorphism and sub-graph matching to find similarities. %All these similarity algorithms are effective at identifying structural duplication or finding recurring patterns.

Recent advancements in this area include neural models and embeddings which integrate both structural and semantic contexts. While embeddings can be compared using the same distance metrics as TF-IDF vectors, such as cosine or Euclidean distance, their strength lies not in the comparison method itself, but in the rich, distributed representations they produce. Capturing deep semantic relationships between SE artifacts, such as source code changes, enables more meaningful comparisons across diverse and lexically varied codebases which makes embeddings very popular in SE. 


Despite their advantages, embeddings have several  limitations~\cite{hellendoorn2017deep}. They do not explicitly model hierarchical structures, that are often essential for understanding source code changes. Additionally, embeddings are inherently opaque: the individual dimensions in an embedding vector lack clear, interpretable meaning. This lack of explainability poses challenges for tasks such as software defect prediction where developers require transparent, explainable insights into why a particular code segment is flagged as defective. Since embeddings are dense, non-interpretable vectors, they do not provide clear reasoning or traceable features behind prediction, making it difficult for developers to validate or trust the results. Additionally, embeddings do not explicitly capture hierarchy inherent in source code such as the relationships between code elements, functions, control flow, which are often crucial for accurately identifying defects. Together, these limitations make embeddings unsuitable for use cases where explainability and structural understanding are essential. Therefore, in this thesis, instead of relying on the default go-to method, embeddings, we explore theoretically well-established and simpler alternatives, such as TF-IDF and tree kernels. With the help of these methods we built explainable software defect prediction models while benefiting from the hierarchical structure of source code.


\subsection{Tree kernels based code similarity} 
...
Explain the shift towards viewing code as natural language and using NLP techniques.\\
...
Software developers have to make critical decisions—debugging, code maintenance, and feature development— at each step of building the software to ensure high quality useful software. As a SE researcher, my goal is to ensure that developer assistance tools to support decision making are not only accurate but also explainable. In fact, transparency and trust is extremely important for the adoption of any real world SE tool. A developer not only wants to know the cause of a bug, or the reason behind a recommended code change but they also want to understand the reason behind such predictions. Prior work~\cite{jiarpakdee2020empirical} has argued that a lack of explainability of defect prediction models could hinder the adoption of defect prediction models in practice. Therefore, explainability is becoming a central focus in SE research, especially in domains like defect prediction, code recommendation, and test case prioritization. If an approach can explain why a file is buggy, why a line was blamed, or why a code change is recommended, it will be called explainable. 
...
Fine-grained JIT-SDP: Either a separate section or merge into SDP section. 

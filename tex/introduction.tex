...
\label{chp:intro}
\textit{Software Defect Prediction (SDP)} is a prominent method in software engineering that aims to guide limited quality assurance resources towards testing components of the software that are most likely to contain defects~\cite{ostrand2004bugs}. These components may include modules, classes, or even software changes. \textit{Just-in-Time software defect prediction (JIT-SDP)} focuses on assessing the risk of software changes at the moment they are committed~\cite{kamei2012large}. This allows potential defects to be flagged while the code is still fresh in the developer's mind~\cite{EMSE6effortaware,huang2017supervised}, making inspection and testing more efficient and cost-effective.
...
%Can structural representations improve model accuracy?
...
Furthermore, SDP models are typically trained offline and are not updated post-deployment, even in the presence of new data because retraining ML and DL models is computationally intensive~\cite{mahadevan2024cost} and often impractical. %Together, these issues result in models that are brittle, outdated, and evaluated under unrealistic assumptions. 
...
\textit{\textbf{Structural Benefit Hypothesis (H-4):} Tree kernel similarity improves code retrieval performance by at least 5\% compared to a token-based approach.}

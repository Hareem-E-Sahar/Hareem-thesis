...
Software defect prediction approaches have not seen widespread adoption in industry~\cite{grattan2024need}\cite{rana2014framework}. Although researchers acknowledge this gap~\cite{grattan2024need}\cite{kim2025reflection}, there is a notable lack of studies that systematically examine the limitations and boundaries of existing methods that hinder their use in real-world settings. This thesis aims to bring some of these issues to the forefront so that they can be addressed by the research community. In particular, my thesis contributes an assessment of the lack of realism, especially with respect to time, in the evaluation and execution of SDP models. In the rest of this section, I highlighting the significance of my contributions, reflect on the methodologies employed, and discuss current limitations.

\section{How does this thesis advance SDP?}	
...
Several studies attribute this instability~\cite{bangash2020time,ekanayake2009} to the software's evolution and the natural shift in the data distribution changes, leading to concept drift~\cite{cabral2019class,tan2015online}. As a result, model behavior changes as well. Evaluations that assume stationary nature of data create optimistic estimates that break under real deployment conditions.
...
This insight motivated the second contribution of my thesis. If model performance is unstable and evaluation must be incremental, then models themselves must be able to operate incrementally. In practice, repeatedly retraining complex ML or DL models such as JITLine and JITFine from scratch is expensive and quickly becomes impractical. 
However, existing studies do not account for the economic cost~\cite{herbold2021exploring} or inefficiency of repeated retraining in their evaluation.
Despite the inherently online nature of JIT SDP, many existing approaches are trained offline and evaluated in static settings.


Contribution 2 (Chapter~\ref{chp:irjit}) of this thesis directly addresses this gap by demonstrating that an online, incrementally updatable defect prediction model is feasible.
%By relying on IR, which updates indices quickly and incrementally, rather than training heavy ML and DL pipelines, IRJIT updates naturally as new commits arrive and avoids costly retraining.
By leveraging IR's fast, incremental indexing instead of resource intensive ML/DL training pipelines, IRJIT can update continuously as new commits arrive, eliminating the need for expensive retraining.
IRJIT was found to be competitive with state-of-the-art ML and DL approaches, JITLine and JITFine, suggesting that more complex models are not necessarily better performing models. My thesis shows that a simpler approach achieves comparable performance while better aligning with the constraints of real development workflows.

The use of IR was a design choice influenced by prior work of Campbell et al~\cite{campbell2016unreasonable}, and it allows me to address another barrier to adoption of SDP models: lack of interpretability. Traditional SDP research largely prioritizes maximizing benchmark performance. However, models optimized solely for performance may not comply with practitioner needs. Developers require predictions that are explainable and ideally actionable~\cite{jiarpakdee2021practitioners}. Decisions obtained from opaque models reduce trust and discourage adoption~\cite{grattan2024need}. By exposing similar historical commits and enabling line-level risk ranking, this contribution of my thesis offers a form of explanation grounded in observable evidence rather than black box model internals.
...
%without sacrificing the model's online and incremental nature. 
...
\subsection{Tree Kernels for source code retrieval}
This work is significant because it addresses a gap in existing source code retrieval techniques by introducing tree kernels as a viable alternative to both token-based and embedding-based approaches. While the software engineering community has made significant strides with embeddings like CodeBERT, these models often fail to preserve critical syntactic structure and struggle with tasks that require nuanced, hierarchical comparisons. Tree kernels, on the other hand, operate directly on the Abstract Syntax Tree (AST), enabling them to capture complex structural relationships between code fragments, such as loops, conditionals, and nested function calls, which are often obscured by token-based methods or flattened by embeddings. This paper’s novel contribution lies not only in demonstrating the potential of tree kernels for ranked code retrieval—a task that closely mirrors real-world use cases—but also in introducing a practical hybrid model that reduces the computational cost of tree kernels, making them feasible for large-scale applications. By positioning tree kernels as a transparent and deterministic approach to structural code search, this work opens the door for future techniques that prioritize interpretability and performance in real-world software engineering tasks, such as bug fixing, refactoring, and code comprehension.
...
\section{Relevance of SDP in the Era of Large Language Models}
Research conducted prior to the appearance of Large Language Models (LLMs) naturally invites the question of whether such work remains relevant in light of the recent advances LLMs have brought to SE. This concern is understandable given the impressive capabilities demonstrated by LLMs~\cite{hou2024large}; however, we cannot overlook the fundamental limitations of LLM-based approaches~\cite{sallou2024breaking,ouyang2025empirical,perkovic2024hallucinations}. In this section, I argue that rather than making SDP obsolete, LLMs introduce new challenges that further reinforce the need for reliable, time-aware SDP models.

\subsection{Faster Code Production and Need for SDP}
One immediate consequence of LLM adoption is the change in the scale at which code is produced~\cite{kumar2025intuition,cui2025effects}, which has direct implications for defect prediction.
LLMs significantly increase the speed and volume of code production~\cite{kumar2025intuition,cui2025effects}. 
...
\subsection{Nature of Defects in LLM Code} 
...
In contrast, traditional SDP models estimate risk using long-term project signals such as developer activity, process metrics~\cite{prem2013}, and prior defect trends~\cite{Jureckzo}, yielding reproducible, auditable, and explainable predictions.

However, LLMs function as probabilistic inference systems whose outputs may vary with prompt formulation~\cite{zhang2025prompt}, model updates, and deployment conditions, limiting their suitability for consistent and verifiable quality assurance. Although LLMs can generate natural-language explanations, their internal decision processes remain opaque~\cite{palikhe2025towards} and hard to validate empirically. These differences indicate that traditional vs LLM-based SDP approaches capture different dimensions of commit riskiness rather than the same signal. As a result, neither approach fully subsumes the other, and their combination may provide more robust and actionable defect risk estimates.
...
%
%SZZ and its variants do not do a great job at labeling~\cite{chakkrit2015mislabel}. Explainability datasets and metrics are not available.
%
